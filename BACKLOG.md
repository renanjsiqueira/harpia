...
- [x] `INTEG-010` **Autenticação básica e response validation de integração** — `DONE`; o client gerado confere a resposta contra o contrato que a porta declarou: um corpo que omite o que a declaração chamou de obrigatório levanta a exceção da porta em vez de chegar como `null` para o flow seguir em frente, e as violações saem ordenadas para que a mesma resposta ruim diga sempre a mesma coisa. A **autenticação** entra por `## Auth` no arquivo de binding, com `bearer` e `api key <Header>`: o esquema é binding porque é parte de como o outro lado é alcançado, e a credencial não é — muda por ambiente e é segredo, então entra por `harpia.integration.<porta>.credential`, placeholder no `application.yaml` gerado. O header é instalado uma vez no builder do client, e não repetido em cada operação, para que exista um só lugar que lê a credencial e nenhuma operação que possa esquecer de mandá-la. Uma mesma porta alcançada de dois arquivos com esquemas diferentes é `HRP2204`, porque o client é um objeto com uma credencial só · `P0` · `L` · Area: `Integration`
  - Evidence: [`IntegrationAuthTest`](src/test/java/dev/harpia/target/javaspring/IntegrationAuthTest.java), [`IntegrationHttpClientTest`](src/test/java/dev/harpia/target/javaspring/IntegrationHttpClientTest.java), [`JavaSpringIntegrationClientTransformer`](src/main/java/dev/harpia/target/javaspring/transformer/JavaSpringIntegrationClientTransformer.java), [`harpia-bindings-v1.md`](docs/spec/harpia-bindings-v1.md).
  - Depends on: `INTEG-001`, `INTEG-002`.
  - Nota: o teste que roda o projeto gerado mostrou que o Spring deixa um placeholder aninhado não resolvido como está — sem `src/test/resources/application.yaml` o client manda a string `${FRAUD_SERVICE_CREDENTIAL}` como se fosse segredo, em vez de o contexto recusar subir. É por isso que a configuração de teste é gerada, e a mesma frouxidão vale para `DATABASE_URL` e para o `jwk-set-uri`: um `harpia.yaml` sem ambiente produz projeto que sobe e mente. Candidato a item próprio.
...
- [x] `DOC-001` **Documentação compacta e exemplos para humanos/LLMs** — `DONE`; [`docs/catalog.md`](docs/catalog.md) é o catálogo único: cada entrada é uma **especificação inteira**, pronta para copiar, e `CatalogTest` compila todas a cada execução — documentação que não pode derivar, porque uma entrada que parou de funcionar quebra o build em vez de enganar quem lê. As entradas marcadas com código mostram o que é **recusado**, e a recusa também é verificada. Escrever o catálogo já encontrou um defeito real: `{id}` fora do último segmento não era reconhecido como o registro que o flow carrega · `P0` · `L` · Area: `CLI/DX`
  - Cobre também controle de acesso: `authenticated`, `role`, `scope` e as recusas que cada um traz. Uma entrada declara o `security.provider` que precisa, porque o que prova uma identidade é escolha de deployment e não parte do exemplo.
