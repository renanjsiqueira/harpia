...
- [ ] `FLOW-011` **`set`, `add` e `remove`** — `TODO` · `P0` · `L` · Area: `API`
...
- [ ] `QUERY-004` **Sorting declarativo** — `PARTIAL`; list-all impõe ordem estável por id, sem DSL de sort · `P0` · `M` · Area: `Persistence`
  - Evidence: [`JavaSpringServiceTransformer`](src/main/java/dev/harpia/target/javaspring/transformer/JavaSpringServiceTransformer.java), [`GeneratedTestTransformerTest`](src/test/java/dev/harpia/target/javaspring/transformer/GeneratedTestTransformerTest.java).
...
- [ ] `API-004` **Path params gerais** — `PARTIAL`; somente `{id}` no último segmento · `P0` · `M` · Area: `API`
...
- [ ] `INTEG-001` **Integration e Operation como portas tipadas** — `TODO` · `P0` · `XL` · Area: `Integration`
...
- [ ] `DOC-001` **Documentação compacta e exemplos para humanos/LLMs** — `PARTIAL`; README bilíngue e specs existem, sem catálogo estruturado único · `P0` · `L` · Area: `CLI/DX`
...
2. `QUERY-004`–`QUERY-005`: sorting declarativo e paginação sobre os filtros de `QUERY-003`.
...
2. Implementar `QUERY-004`–`QUERY-005` para SearchOrders.
