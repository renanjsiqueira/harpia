...
- [ ] `INTEG-010` **Autenticação básica e response validation de integração** — `PARTIAL`; response validation pronta. O client gerado confere a resposta contra o contrato que a porta declarou: um corpo que omite o que a declaração chamou de obrigatório levanta a exceção da porta em vez de chegar como `null` para o flow seguir em frente, e as violações saem ordenadas para que a mesma resposta ruim diga sempre a mesma coisa. Falta a **autenticação** (API key/bearer), que é credencial e portanto configuração, não declaração · `P0` · `L` · Area: `Integration`
